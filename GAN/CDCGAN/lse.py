...
'''
Code for latent space exploration using CMA-ES
'''
...
def normalize_fitness(x, lower_bound=-1.0, upper_bound=1.0):
    '''
    Function to normalize fitness values 
    '''
    if x < -100:
        x = -100
    elif x > 100:
...
def matrix_eval_simple(level):
    '''
    Simple fitness function based on number of sky and ground tiles
    '''
...
def matrix_eval(level):
    '''
    Fitness function that gathers numerous tile frequencies in certain regions of the level, 
    and uses these metrics to optimize for different structural features in the level
    '''
...
def evaluate_kl(ori_lvl, gen_lvl):
    '''
    Compute KL divergence loss of the tile distribution
    '''
...
if __name__ == "__main__":
    ...
    # edit it this with the specific path to load correct level
    ...
    # fitness functions used to get the best soltuion (level)
    while count <= 1000:
