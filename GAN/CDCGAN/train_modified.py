...
from additional_cond import count_sky_tiles, count_ground_tiles, count_enemy_tiles


# Train sky tile CDCGAN. Enables latent space exploration.

def parse_arguments():
    '''
    Allows command line arguments for GAN hyperparameters
    Hyperparameters include: latent z size, batch size, number of iterations, learning rates, etc.
    '''
    ...
    parser.add_argument(
        "--niter", type=int, default=50000, help="number of epochs to train for"
    )
    parser.add_argument(
        "--lrD",
        type=float,
...
def combine_images(generated_images):
    ...
    generated_images is the output of tiles2image
    '''
...
def train(
    ...
    for epoch in range(opt.niter):
        ...
        while i < num_batches:  # len(dataloader):
            ...
            while j < Diters and i < num_batches:  # len(dataloader):
                ...
                true, false = count_enemy_tiles(joined_frame)
                ...
                input.resize_as_(true_joined_frame).copy_(
                    true_joined_frame)
                ...
                false_joined_frame = joined_frame[false]
                stitched = torch.cat(
            ...
            # 0430/2023
            print(
                "[%d/%d][%d/%d][%d] Loss_D: %f Loss_G: %f Loss_D_real: %f Loss_D_fake %f Loss_D_fake_1 %f Loss_D_fake_2 %f"
                % (
                    epoch,
                    opt.niter,
                    i,
            ...
            loss_row = torch.tensor([epoch,errD.data[0],errG.data[0],errD_real.data[0],errD_fake.data[0],errD_fake_1.data[0],errD_fake_2.data[0]])
            if len(loss_value) ==0:
            ...
            if gen_iterations % 10000 == 0:  # was 500
                ...
                torch.save(
                    netG.state_dict(),
                    "{0}/netG_epoch_condition_012567_enemy_{1}_{2}_{3}.pth".format(
    ...
    np.save('loss_data_enemy.npy', loss_value)



def main():
    # take in hyperparameter arguments
    opt = parse_arguments()
